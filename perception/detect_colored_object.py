...
while True:
    ...
    erode_kernel = cv2.getStructuringElement(
        erosion_shape,
        (2 * erode_kernel_size + 1, 2 * erode_kernel_size + 1),
    ...
    dilate_kernel = cv2.getStructuringElement(
        dilate_shape,
        (2 * dilate_kernel_size + 1, 2 * dilate_kernel_size + 1),
    ...
    contours, hierarchy = cv2.findContours(
        bin_dilate_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    if max_x - min_x > 0 and max_y - min_y > 0:
        cv2.rectangle(
            output_img, (min_x, min_y), (max_x, max_y), (255, 0, 0), 2
        )
    ...
    cv2.moveWindow("Mask", 1250, 0)
    cv2.imshow("erode", bin_erode_img)
