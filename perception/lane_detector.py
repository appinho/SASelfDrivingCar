...
class LineDetector:
    ...
    def abs_sobel_thresh(
        self, gray, orient="x", sobel_kernel=3, thresh=(0, 255)
    ):
        ...
        grad_binary[
            (scaled_sobel >= thresh[0]) & (scaled_sobel <= thresh[1])
        ] = 1
    ...
    def mag_thresh(self, gray, sobel_kernel=3, thresh=(0, 255)):
        ...
        mag_binary[
            (scaled_magn >= thresh[0]) & (scaled_magn <= thresh[1])
        ] = 255
        # 6) Return this mask as your binary_output image
        return mag_binary
    ...
    def find_polynomial(self, image, binary_warped):
        ...
        if len(self.history_left_line) == 1:
            ...
            self.state_left_line.append(
                self.smooth_factor * self.state_left_line[-1]
                + (1 - self.smooth_factor) * leftx_base
            )
        if len(self.history_right_line) == 1:
            self.state_right_line.append(rightx_base)
        else:
            self.state_right_line.append(
                self.smooth_factor * self.state_right_line[-1]
        ...
        if self.debug:
            visualizer.plot_polynomial_fit(
                binary_warped, out_img, left_fitx, right_fitx, ploty
            )
        ...
        left_curverad = (
            (
                1
                + (
                    2 * left_fit_cr[0] * y_eval * self.ym_per_pix
                    + left_fit_cr[1]
                )
                ** 2
            )
        ...
        right_curverad = (
            (
                1
                + (
                    2 * right_fit_cr[0] * y_eval * self.ym_per_pix
                    + right_fit_cr[1]
                )
                ** 2
            )
        ...
        if len(self.history_left_curvature) == 1:
            ...
            self.state_left_curvature.append(
                self.smooth_factor * self.state_left_curvature[-1]
        ...
        if len(self.history_right_curvature) == 1:
            ...
            self.state_right_curvature.append(
                self.smooth_factor * self.state_right_curvature[-1]
    ...
    def process_image(self, image):
        ...
        binary_image = self.create_thresholded_image(image)
        if self.debug:
            pass
        ...
        src = np.float32(
            [[560, 475], [205, img_size[0]], [1105, img_size[0]], [724, 475]]
        )
        dst = np.float32(
            [
                [(img_size[1] / 4), 0],
        ...
        result = visualizer.get_result(
            image, topdown_image, pts, Minv, image, 100, -0.23
        )
    ...
    def adjust_parameters(self, image):
        ...
        while True:
            ...
            cv2.moveWindow("Parameter", 800, 400)
            ...
            mag_binary = self.mag_thresh(
                gray,
                sobel_kernel=self.kernel_size * 2 + 1,
                thresh=(g_min, g_max),
            )
